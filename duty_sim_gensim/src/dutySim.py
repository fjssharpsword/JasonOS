...
def parse_args():#define the paramter of program
    ...
    parser.add_argument('--senName', nargs='?', default='../data/sen.txt',help='text.')
...
if __name__ == '__main__':
    #1.读取参数
    args = parse_args()
    senName = args.senName
    topK = args.topK
    #2.分词
    text=[line for line in open('../data/sen.txt', 'r',encoding='GBK').readlines()]
    lineText = get_lineText(text[0])
    #3.匹配相似度最高k项
    mostSim = get_most_similar(lineText,topK)
    rawText = [line for line in open('../data/raw.txt', 'r',encoding='GBK').readlines()]
    simItems = []
    for i, sim in mostSim:
        strtxt = rawText[int(i)]
        simItems.append([i,sim,strtxt])
    with open('../data/sim.txt','w',encoding='GBK') as fw: #返回最高相似度结果
